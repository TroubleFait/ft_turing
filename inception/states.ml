...
let state_of transition alphabet : string * Rules.state =
  name transition,
  alphabet
  |> String.fold_left (fun acc read ->
    (read, make_transition (read, transition)) :: acc ) []
  |> List.to_seq
...
let to_state_map assoc transitions : Rules.action list CharMap.t CharMap.t =
  ...
  Utils.StringHash.fold (fun s state acc -> Printf.printf "Utils.StringHash.fold s: %s\n" s;
    Utils.CharHash.fold (fun _ (transition: Rules.transition) acc ->
      (char_of_state transition.to_state, (transition.write, transition.action)) :: acc ) state acc
    ) transitions []
...
let to_states tsmap =
  CharMap.fold (fun ts _ l -> ts :: l) tsmap [] |> string_of_list

let writes wmap =
  CharMap.fold (fun w _ l -> w :: l) wmap [] |> string_of_list

let actions alist =
  String.init (List.length alist) (fun i -> List.nth alist i |> Encoder.char_of_action)
...
let list_states (context: context) =
  ...
  let by_to_state make_state =
    to_state_map |> CharMap.bindings |> List.iter (fun (ts, _) -> Printf.printf "to_state:%c\n" ts);
    CharMap.fold (fun ts wmap -> make_state ts wmap) to_state_map
  in
  let by_write make_state =
    by_to_state (fun ts wmap ->
      CharMap.fold (make_state ts) wmap)
  in
  let by_action make_state =
    by_write (fun ts w alist lst ->
      List.fold_left (fun lst a -> make_state ts w a lst) lst alist)
  in
  [                       state_of Initial                (states ^ "H") ]
  |> by_states  (fun s -> state_of (Go_B  s)              (states ^ tape ^ "F|HLRB"))
  |> by_states  (fun s -> state_of (Put_C s)              (tape ^ "BE"))
  |> by_states  (fun s -> state_of (Put_E s) (String.of_char context.rules.blank))
  ...
  |> by_to_state (fun ts wmap l  -> (state_of (Get_write ts)        (writes wmap)) :: l)
  |> by_write  (fun ts w alist l -> (state_of (Get_action (ts, w))  (actions alist)) :: l)
  |> by_action (fun ts w a l     -> (state_of (Go_C_put (ts, w, a)) (states ^ tape ^ "|HLRBC")) :: l)
